...
1. 回測顯示所有做多策略均虧損（PF < 1.0）→ 優先建議偏空或觀望，但如果技術面有明確做多信號（RSI 極端低、MACD 底背離、觸及關鍵支撐），可建議小倉位試單（≤10% 資金）並附帶止損
2. 回測顯示所有做空策略均虧損（PF < 1.0）→ 優先建議偏多或觀望，同理技術面有明確信號時可小倉位試單
3. 回測全部虧損 → 預設建議觀望，但如果技術面有明確方向性信號，可建議小倉位試單（≤10% 資金），必須附帶具體止損位和風險警告
4. 報告中必須引用回測關鍵數據（勝率、PF、Sharpe、MDD、交易次數）
5. 必須說明回測使用了多少根 K 線（回測樣本量）

如果你的技術面分析方向與回測結果矛盾，坦誠說明：
「技術面訊號偏多/偏空，但歷史回測不完全支持。建議小倉位操作並嚴格控制止損。」
注意：回測虧損可能是因為成本模型或參數不匹配，不代表方向一定錯誤。
