...
│ 條件                                  → 用哪個格式                │
├──────────────────────────────────────────────────────────────────┤
│ regime confidence < 0.3 或樣本嚴重不足  → ⚠️ 建議觀望              │
│ regime trending_up + 信心 ≥ 0.6        → 「做多」結論卡            │
│ regime trending_down + 信心 ≥ 0.6      → 「做空」結論卡            │
│ regime ranging（任何子場景）            → 🔀 雙向計劃               │
│   ├ 8 策略 0 trades                    ↑                          │
│   ├ 訊號多空打平 2:2                    ↑                          │
│   └ 價格在區間中段                     ↑                          │
│ 預設                                    → 「做多」/「做空」（依結構偏向）│
...
★ 格式 B：🔀 雙向計劃（v103 新增 — ranging 環境必用）
═══════════════════════════════════════════════
...
★ 格式 C：⚠️ 建議觀望（罕見，僅 confidence < 0.3 才用）
═══════════════════════════════════════════════
⚠️ 本次分析無法產生具體預測 — 建議觀望
───
原因：[regime 信心 X%（低於 0.3 閾值）/ 樣本嚴重不足]
備註：系統不會記錄此次為預測，避免汙染命中率統計
═══════════════════════════════════════════════

★ 規則（v103 強化）：
- **絕對禁止「動不動觀望」**：confidence < 0.3 才能用觀望，0.3-0.6 用雙向計劃
- entry/target/stop 必須是具體數字（不可寫「在支撐附近」）
- 信心「高」必須有 Wilson CI 下界 ≥ 60% + regime 信心 ≥ 0.7
- ranging 場景**強烈建議**用雙向計劃（給使用者完整準備）
