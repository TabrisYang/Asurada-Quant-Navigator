...
def train_imitation_model(min_samples: int = 50, force: bool = False) -> dict:
    ...
    if overfit_gap > 0.20:  # 放寬 0.15 → 0.20，因為小樣本時 train AUC 容易高
        _save_metrics_to_db(metrics, status="rejected_overfit", is_champion=False, failed_reasons="overfit_gap > 0.20")
        return {"status": "rejected_overfit", **metrics}

    if lockbox_auc < 0.55:
        _save_metrics_to_db(metrics, status="rejected_lockbox_random", is_champion=False, failed_reasons="lockbox_auc < 0.55")
        return {"status": "rejected_lockbox_random", **metrics}

    # ─── Champion-Challenger：新模型 AUC 沒比現役高 0.02 不切換 ───
    current_champion = _get_champion_metrics()
    ...
    # ─── 通過所有檢查 → 啟用 ───
    version = _save_metrics_to_db(metrics, status="activated", is_champion=True, failed_reasons=None)

    # 把舊 champion 改為 stable_fallback（保留退路）
    _demote_old_champion(version)

    # 儲存模型 + SHAP explainer
    model_path = _MODELS_DIR / f"imitation_v{version}.pkl"
    ...
    try:
        ...
        joblib.dump(plain_base, _MODELS_DIR / f"imitation_v{version}_plain.pkl")
...
def get_active_model() -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(f"載入 model v{version} 失敗：{e}")
        return None


def get_plain_model_for_shap(version: int) -> Optional[Any]:
...
def _load_training_data() -> pd.DataFrame:
    ...
    """
    cursor = prediction_tracker._conn.execute(q)
...
def _get_champion_metrics() -> Optional[dict]:
    """取目前 champion 模型的 metrics。"""
    prediction_tracker._ensure_db()
    if not prediction_tracker._conn:
        return None
    row = prediction_tracker._conn.execute(
        """SELECT version, trained_at, trainset_n, auc, train_auc, lockbox_auc, brier, overfit_gap
           FROM imitation_model_metrics WHERE is_champion = 1
    ...
    cursor = prediction_tracker._conn.execute(
        """INSERT INTO imitation_model_metrics
           (trained_at, trainset_n, auc, train_auc, lockbox_auc, brier, overfit_gap,
            ...
            quality_gate_passed, quality_gate_failed_reasons)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        (
...
def _demote_old_champion(new_version: int) -> None:
    ...
    prediction_tracker._conn.execute(
        """UPDATE imitation_model_metrics
           ...
           WHERE is_champion = 1 AND version != ?""",
        (new_version,),
    )
    prediction_tracker._conn.commit()
