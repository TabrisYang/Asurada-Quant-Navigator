...
使用 PyTorch 實作，將特徵序列輸入 LSTM 網路進行分類。
"""
...
@model_registry.register(
    ...
    name="LSTM",
    category="神經網路",
    description="長短期記憶網路，擅長捕捉時序依賴關係，適合有明顯時序模式的市場",
    requires=["torch"],
...
class LSTMPredictor(BasePredictor):
    ...
    def train(
        ...
        # 轉換為序列格式 (samples, seq_len, features)
        ...
        logger.info(
            f"LSTM 訓練完成: OOS acc={result.oos_accuracy:.3f} "
            f"AUC={result.oos_auc:.3f} ({elapsed_ms}ms, {epoch+1} epochs)"
        )
        return result

    def predict(self, X: np.ndarray) -> np.ndarray:
        """X 已是 (n_samples, seq_len, n_features) 或 (n_samples, n_features)"""
...
def _make_sequences(
    ...
    """將 (n, features) 轉成 (n-seq_len+1, seq_len, features)"""
    n = len(X)
    if n < seq_len:
...
class _LSTMNet:
    """純 Python 類別，延遲到 train/load 時才用 torch.nn"""
