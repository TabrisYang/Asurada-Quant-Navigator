"""阿斯拉量化系統 — Transformer 預測模型

使用 PyTorch 實作輕量級 Transformer Encoder 進行時序分類。
"""
...
@model_registry.register(
    ...
    name="Transformer",
    category="神經網路",
    description="注意力機制模型，擅長捕捉長距離依賴關係，數據量充足時表現優異",
    requires=["torch"],
...
class TransformerPredictor(BasePredictor):
    ...
    def train(
        ...
        if len(X_train_seq) < 30 or len(X_val_seq) < 10:
            logger.warning("Transformer: 序列化後樣本不足")
        ...
        model = _build_transformer(
            n_features, seq_len,
            cfg["d_model"], cfg["nhead"], cfg["num_layers"],
        ...
        logger.info(
            f"Transformer 訓練完成: OOS acc={result.oos_accuracy:.3f} "
    ...
    def get_feature_importance(self) -> dict[str, float]:
        return dict(
            sorted(self._importance_scores.items(), key=lambda x: x[1], reverse=True)
        )
    ...
    def load(self, path: Path) -> None:
        ...
        self._model = _build_transformer(
            n_features, cfg["sequence_length"],
            cfg["d_model"], cfg["nhead"], cfg["num_layers"],
            cfg["dim_feedforward"], cfg["dropout"],
        )
...
def _build_transformer(
    ...
    """工廠函式，延遲 import torch"""
    import torch
    import torch.nn as nn

    class PositionalEncoding(nn.Module):
        def __init__(self, d_model_: int, max_len: int = 500):
            super().__init__()
            pe = torch.zeros(max_len, d_model_)
            position = torch.arange(0, max_len, dtype=torch.float).unsqueeze(1)
            div_term = torch.exp(
                torch.arange(0, d_model_, 2).float() * (-math.log(10000.0) / d_model_)
            )
            pe[:, 0::2] = torch.sin(position * div_term)
            if d_model_ % 2 == 0:
                pe[:, 1::2] = torch.cos(position * div_term)
            else:
                pe[:, 1::2] = torch.cos(position * div_term[:d_model_ // 2])
