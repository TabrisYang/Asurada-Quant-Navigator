...
async def _run_sync(request: DataSyncRequest, task_id: str):
    ...
    try:
        for symbol in request.symbols:
            for tf in request.timeframes:
                ...
                try:
                    ...
                    # ML: 遞增 bars 計數器（受 ML_ENABLED 開關控制）
                    try:
        ...
        # ML: 同步完成後驗證預測 + 檢查自動重訓（受 ML_ENABLED 開關控制）
        retrain_count = 0
        validated_count = 0
        try:
            from app.core.ml._settings import ML_ENABLED
...
async def get_tw_stock_name(code: str):
    ...
    try:
        ...
        if static_name and static_name != code:
            _tw_name_cache[code] = static_name
            return {"code": code, "name": static_name}
    except Exception:
        pass
