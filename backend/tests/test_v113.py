"""v113 regression tests：防止 LLM CLI subprocess 行間超時退回 60 秒

（v151 訂閱 adapter 重寫後變數更名：_line_timeout → _IDLE_TIMEOUT，語意不變 —
已開始輸出後行與行間的最大等待秒數）。

歷史 bug（v107~v112 隱藏）：
adapter.py 三處 `_IDLE_TIMEOUT = 60` 對「全部分析」太短 — 全部分析 Round 2
...
def _adapter_src() -> str:
    # v154 拆檔後 CLI 逐行讀取邏輯在 adapters/claude_subscription.py；
    # 合併整個 adapters/ package 原始碼掃描，未來再搬家也不會漏
    pkg = pathlib.Path(__file__).resolve().parent.parent / "app" / "core" / "llm" / "adapters"
...
def test_IDLE_TIMEOUT_at_least_300():
    """確認 adapter.py 中所有 `_IDLE_TIMEOUT = N` 賦值 N >= 300，不再是 60。"""
    src = _adapter_src()
    matches = re.findall(r"_IDLE_TIMEOUT\s*=\s*(\d+)", src)
    assert matches, "adapter.py 找不到 _IDLE_TIMEOUT 賦值（測試 grep 失效或變數已被刪除）"
    for val_str in matches:
        val = int(val_str)
        assert val >= 300, (
            f"adapter.py 出現 _IDLE_TIMEOUT = {val}（< 300）"
            f"——歷史 bug 復活，會導致全部分析報告中斷。修回 >= 300。"
        )


def test_IDLE_TIMEOUT_smaller_than_frontend_stream_timeout():
    ...
    backend_vals = [int(v) for v in re.findall(r"_IDLE_TIMEOUT\s*=\s*(\d+)", backend_src)]
    assert backend_vals, "找不到 backend _IDLE_TIMEOUT 設定"
    ...
    assert backend_max_seconds < frontend_seconds, (
        f"backend _IDLE_TIMEOUT 最大值 {backend_max_seconds}s "
