...
### v143 — P3 強制熔斷取消（離線回測否決，治理教訓）

**結論**：原 plan 的 P3（regime-aware 強制熔斷層，「衝突場景強制 WAIT」）**取消，不實作**。

**為何**：用 `shadow_mode --days 400` 重放 259 筆全歷史做離線回測，發現 P3 想熔斷的「衝突場景」在歷史上**實際是賺錢的**：

| 觸發 | 樣本 | 命中率 | 平均報酬 |
|---|---|---|---|
| C：ranging + 有方向 | 125 | 70.2% | +1.90% |
| F：biased 順勢 | 16 | 66.7% | +0.68% |
| A：三防線同時失效 | 0 | — | （全歷史從沒觸發）|

若 P3 對這些場景強制 WAIT/REDUCE，會砍掉 125 筆勝率 70% 的賺錢交易 —— 這些正是 PF 1.45 alpha 的一部分。**強制熔斷會減損 alpha，不是保護。**

**關鍵教訓（給未來接手者）**：
1. **小樣本會誤導**：5/19 用 `--days 14`（僅 14 筆、剛好是 ADA 弱勢期 + buckets bug 期）顯示「衝突場景虧錢、判定 bug」；但 `--days 400`（259 筆全歷史）完全翻轉成「賺錢、建議 A 門檻調嚴」。**動核心邏輯前，務必用足夠大的歷史樣本回測，不要被近期小窗口騙。**
2. **這是 CLAUDE.md「動核心邏輯前先跑 shadow mode」紀律的實證價值** —— 它在寫任何 code 前就攔下了一個會傷 alpha 的改動。
3. **既有軟性 veto/觀望邏輯（veto_active / win_rate<40% / 觀望卡）已足夠**，且不會硬殺賺錢交易。不需要再疊加程式級強制熔斷。

**保留項**：v141.3 的「三重否決強制 WAIT」（MC破產≥50% + WF❌ + CPCV❌）暫保留 —— 那是針對「單一回測策略不可部署」的不同訊號，非 regime 衝突。日後可用同樣 `--days 400` 方法驗證再決定去留。

**衍生發現（足樣本永遠卡住的根因）**：觀望結論不產生預測（parse_predictions 命中觀望卡 → 回 []），低信心預測被過濾。使用者若一直分析弱勢盤整標的（ADA），系統正確判觀望 → 不存預測 → 足樣本不累積。這是正確的 alpha 紀律，非 bug。
